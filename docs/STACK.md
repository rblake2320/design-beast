...
| Media library organizer | `beast library` — SigLIP2 SO400M (`google/siglip2-so400m-patch14-384`, HF cache `D:\Models\huggingface`), InsightFace buffalo_l (`~\.insightface`), imagehash, easyocr, Ollama qwen3-vl:8b / qwen3.8:27b / bge-m3; store = SQLite (`library/data/`) or pgvector (`BEAST_LIBRARY_DSN`; dev container `beast-library-pg` :5436); batched fast tier = container `beast-vllm` :8021 (`vllm/vllm-openai`, Qwen3-VL-8B FP8, needs `VLLM_USE_V2_MODEL_RUNNER=0 VLLM_WSL2_ENABLE_PIN_MEMORY=1` on Docker Desktop) — 7.6 img/s vs Ollama 0.96 | ✅ reproduced 2026-09-12 on 399 real photos (docs/LIBRARY.md) |
